...
without having to wait to reload the database each time. However,
...
def make_parser():
    ...
    parser = argparse.ArgumentParser(
        description="""
                    Explore past and future close approaches
    ...
    filters.add_argument('--max-distance', dest='distance_max', type=float,
                         help="In astronomical units. Only return close "
                              "approaches that pass as near or nearer to Earth"
...
def query(database, args):
    ...
    :param args: All arguments from the command line, as parsed by the
        top-level parser.
    """
    # Construct a collection of filters from arguments supplied
    # at the command line.
    ...
    def precmd(self, line):
        ...
        if changed:
            ...
            if not self.aggressive:
                ...
                print("Preemptively terminating the session "
                      "aggressively.", file=sys.stderr)
                return 'exit'
        return line
